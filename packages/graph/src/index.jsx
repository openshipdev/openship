...
export function SystemGraph({ system, selectedNodeId, onSelectNode, onOpenContext, className = "" }) {
  ...
  const defaults = useMemo(() => gridPositions(model.cards), [model]);
  const fit = () => flow.current?.fitView({ padding: 0.12, duration: 200 });

  useEffect(() => {
    generation.current += 1;
    setPositions(null); setBusy(false); setError("");
  ...
  return <section ref={shell} className={`osg ${className}`} aria-label={`${system.name} architecture graph`}>
    ...
    <div className="osg-canvas"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onInit={(instance) => { flow.current = instance; }} fitView minZoom={0.05} maxZoom={2.5} nodesDraggable={false} nodesConnectable={false} edgesFocusable={false} nodesFocusable={false} elementsSelectable={false} deleteKeyCode={null}>
      <Background gap={14} size={1} color="#9aa5b533" /><Controls showInteractive={false} />
    </ReactFlow></div>
    <div className="osg-status" role="status">{error || `${model.included.size} components · ${edges.length} connections · Scroll to zoom, drag the canvas to pan.`}{model.cards.length === 0 && " No components match these filters."}</div>
  </section>;
}
