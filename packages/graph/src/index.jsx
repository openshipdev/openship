...
import { autoLayout, buildGraph, connectionLabel, DEFAULT_FILTERS, gridPositions, graphSelection, invalidateRoutes, updateMeasurements } from "./model.js";

function selectionClass(id, selected, neighbors) {
  return id === selected ? "osg-selected" : neighbors.has(id) ? "osg-neighbor" : "";
}

function ComponentRow({ node, selected, neighbors, onSelect, onContext, nested = false, row }) {
  return <div className={`osg-component ${nested ? "osg-nested" : ""} ${selectionClass(node.id, selected, neighbors)}`} onClick={(event) => { event.stopPropagation(); if (!event.target.closest("button")) onSelect?.(node.id); }} style={row ? { position: "absolute", top: row.top, left: row.left, width: 288, height: row.height } : undefined}>
...
function Card({ data }) {
  ...
  return <div className={`osg-card ${selectionClass(data.node.id, data.selected, data.neighbors)}`}>
    ...
    {data.routes.length > 0 && <svg className="osg-internal-connections" width={data.width} height={data.height} aria-label="Connections within this host">
      <defs><marker id={markerId} viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 8 4 L 0 8 Z" fill="var(--osg-muted)" /></marker><marker id={`${markerId}-selected`} viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 8 4 L 0 8 Z" fill="var(--osg-selection)" /></marker></defs>
      {data.routes.map((route) => <g key={route.id}>
        <title>{`${route.fromNodeId} → ${route.toNodeId}: ${route.label}`}</title>
        <path className={`osg-internal-path ${data.connectedEdges.has(route.id) ? "osg-connected" : ""}`} d={route.path} markerEnd={`url(#${markerId}${data.connectedEdges.has(route.id) ? "-selected" : ""})`} strokeDasharray={route.type === "Dependency" ? "5 4" : undefined} />
      </g>)}
      {data.routes.map((route) => <foreignObject key={route.id} x={route.labelX} y={route.labelY - 10} width={route.labelWidth} height="20"><div className="osg-internal-labels"><div className={`osg-internal-label ${data.connectedEdges.has(route.id) ? "osg-connected" : ""}`} title={route.label}>{route.label}</div></div></foreignObject>)}
    </svg>}
  </div>;
}
function Root({ data }) {
  return <div className={`osg-root ${selectionClass(data.node.id, data.selected, data.neighbors)}`}><ComponentRow {...data} /></div>;
}
...
export function SystemGraph({ system, selectedNodeId, onSelectNode, onOpenContext, toolbarControls, className = "" }) {
  ...
  const selected = selectedNodeId === undefined ? localSelection : selectedNodeId;
  const selectNode = React.useCallback((id) => { setLocalSelection(id); onSelectNode?.(id); }, [onSelectNode]);
  ...
  const { neighbors, connectedEdges } = useMemo(() => graphSelection(model, selected), [model, selected]);
  ...
  const arrangeAutomatically = useEffectEvent(() => arrange());
  ...
  useEffect(() => {
    ...
    if (!flowReady) return;
    // Let the canvas settle after initialization or a fullscreen resize.
    // Read the latest layout without rerunning this effect on selection or drag.
    const timer = setTimeout(() => {
      if (run === generation.current) arrangeAutomatically();
    }, 100);
    return () => { generation.current += 1; clearTimeout(timer); };
  }, [model, fullscreen, flowReady]);
  useEffect(() => {
    const update = () => {
  ...
  const nodes = useMemo(() => {
    const common = { selected, neighbors, connectedEdges, onSelect: selectNode, onContext: onOpenContext };
  ...
  }, [model, defaults, layout, positions, measurements, selected, neighbors, connectedEdges, selectNode, onOpenContext, system.rootNodeId]);
  const edges = useMemo(() => model.edges.filter((edge) => edge.source !== edge.target || edge.source === system.rootNodeId).map((edge) => ({
    ...
    markerEnd: { type: MarkerType.ArrowClosed, color: connectedEdges.has(edge.id) ? "var(--osg-selection)" : "#8592a3" },
    style: { stroke: connectedEdges.has(edge.id) ? "var(--osg-selection)" : "#8592a3", strokeWidth: connectedEdges.has(edge.id) ? 2.5 : 1.5, strokeDasharray: edge.type === "Dependency" ? "5 4" : undefined },
    labelStyle: { fill: connectedEdges.has(edge.id) ? "var(--osg-selection)" : "var(--osg-fg)", fontSize: 10, fontFamily: "system-ui, sans-serif" }, labelBgStyle: { fill: "var(--osg-bg)" },
  })), [model, system.rootNodeId, layout, connectedEdges]);
  ...
  return <section ref={shell} className={`osg ${className}`} aria-label={`${system.name} architecture graph`}>
    ...
    <div className="osg-canvas"><ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onNodeClick={(_event, node) => selectNode(node.id)} onInit={(instance) => { flow.current = instance; setFlowReady(true); }} fitView minZoom={0.05} maxZoom={2.5} nodesDraggable onNodesChange={moveNodes} onNodeDragStart={() => { generation.current += 1; setBusy(false); }} nodesConnectable={false} edgesReconnectable={false} edgesFocusable={false} nodesFocusable={false} elementsSelectable={false} deleteKeyCode={null}>
      <Background gap={14} size={1} color="#9aa5b533" /><Controls showInteractive={false} />
    </ReactFlow></div>
    <div className="osg-status" role="status">{error || `${model.included.size} components · ${model.edges.length} connections · Drag cards to arrange · Click a node to highlight its connections · Scroll to zoom, drag the canvas to pan. Layout changes stay in this viewer.`}{model.cards.length === 0 && " No components match these filters."}</div>
  </section>;
}
