...
export function resolveSelection(params, snapshot) {
  ...
  const view =
    requestedView === "system"
      ...
      : requestedView === "sources" ? "sources" : "summary";
  const defaultLayer =
    system?.layers.find((item) => item.role === "provider") ??
    system?.layers[0];
  const useDefaults = !params.has("layer") && !params.has("instance");
  const instance =
    system?.instances?.find((item) => item.id === params.get("instance")) ??
    (useDefaults
      ? system?.instances?.find(
          (item) =>
  ...
  const layer =
    ...
    ) ?? (params.has("layer") ? system?.layers[0] : defaultLayer);
...
export async function loadArchivedProvider(value, input = {}) {
  ...
  return loadSavedSnapshot({
    origin,
    retrievedAt: project.archive.retrievedAt,
    hasSystems: project.archive.resources.some((r) => r.kind === "systems"),
    get,
  });
}

// Shared verification for browser archives and server-side DB/R2 snapshots.
