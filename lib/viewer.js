...
export function resolveSelection(params, snapshot) {
  ...
  const defaultLayer =
    system?.layers.find((item) => item.role === "provider") ?? system?.layers[0];
  ...
  ) ?? (useDefaults ? system?.instances?.find(
    (item) => item.layerId === defaultLayer?.id && item.environment === "production",
  ) : undefined);
  const layer =
    system?.layers.find(
