...
export async function listProjects({
  ...
  if (q)
    conditions.push(
      or(
        ...
        ilike(projects.productDescription, `%${q.slice(0, 200)}%`),
        ilike(projects.technicalDescription, `%${q.slice(0, 200)}%`),
  ...
  const [total] = await db
    ...
    .where(where);
  const rows = await db
    .select({ ...getTableColumns(projects), oshHash: snapshots.contentHash })
    .from(projects)
    .leftJoin(snapshots, eq(snapshots.id, projects.latestSnapshotId))
    .where(where)
    .orderBy(
      filter === "home"
